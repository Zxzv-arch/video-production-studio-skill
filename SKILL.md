...
- If the user requires every sentence to have dynamic text, a corresponding PiP state, and an animated Remotion presentation/demo: read [references/sentence-reactive-explainers.md](references/sentence-reactive-explainers.md) in addition to the talking-head references, and treat it as an explicit dense-visual mode.
- Product structure, assembly, spatial mechanisms, 3D data, CAD, GLB/glTF, Three.js, or Text-to-CAD: read [references/threejs-and-cad.md](references/threejs-and-cad.md). Activate 3D only when depth communicates meaning; keep Text-to-CAD optional and validate generated geometry before presenting it as evidence.
...
3. Build a semantic outline: hook, setup, development, proof, payoff, and call to action. Map each spoken claim to one visual purpose. Run a capability design pass before writing scenes: choose the strongest appropriate Remotion treatment or a better specialist engine for each beat, record why it helps, and define an honest fallback.
   ...
   In sentence-reactive mode, validate that every transcript sentence has all three required tracks: dynamic type, a presenter/PiP layout state, and a content-specific Remotion presentation or demonstration with a resolved state.
